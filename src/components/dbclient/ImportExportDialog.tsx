...
function ExportDialog({ open: isOpen, onClose, sessionId, connectionConfig, selectedDatabase, defaultTables }: Props) {
  ...
  const doExport = async () => {
    ...
    try {
      ...
      setResult({ ok: true, msg: t("dbClient.exportComplete", "导出完成：{{size}}，耗时 {{ms}}ms", { size: fmtB(res.sizeBytes), ms: res.durationMs }) });
        ...
        <div className="flex rounded-xl overflow-hidden max-h-[70vh]">
          ...
          <div className="w-[220px] shrink-0 border-r border-border-default bg-bg-secondary flex flex-col min-h-0">
            <div className="flex items-center justify-between px-3 py-1.5 border-b border-border-default shrink-0">
              <span className="text-xs font-semibold uppercase tracking-wider text-text-tertiary">{t("dbClient.exportTables", "导出表")}</span>
            ...
            <div className="px-3 py-1 border-b border-border-default/30 shrink-0">
              <label className="flex items-center gap-2 text-xs text-text-secondary cursor-pointer select-none">
                ...
                {t("dbClient.selectAllNone", "全选 / 全不选")}
              </label>
            </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="p-4 pb-3 space-y-3">
              <Fld label={t("dbClient.exportFormat", "导出格式")}>
              ...
              <Fld label={t("dbClient.outputFile", "输出文件")}>
                ...
                <>
                  <div className="flex gap-5">
                    ...
                    <label className="flex items-center gap-2 text-xs text-text-secondary cursor-pointer select-none">
                      ...
                      {t("dbClient.schemaOnly", "仅结构")}
                    </label>
                  </div>
                  <div>
                    <button onClick={() => setShowAdv(!showAdv)} className="flex items-center gap-1 text-xs text-text-tertiary hover:text-text-primary">
                      {showAdv ? <ChevronDown size={12} /> : <ChevronRight size={12} />} {t("dbClient.advancedOptions", "高级选项")}
                    </button>
                    {showAdv && (
                      <div className="mt-2 ml-4 border-l-2 border-border-default/30 pl-3 space-y-1">
                        <label className="block text-[11px] text-text-tertiary">{t("dbClient.toolPathHint", "工具路径（留空自动检测）")}</label>
                        <div className="flex gap-2">
              ...
              <button onClick={doExport} disabled={running || !path}
                ...
                {running ? <Loader2 size={14} className="animate-spin" /> : <Download size={14} />} {t("dbClient.startExport", "开始导出")}
              </button>
            </div>
          </div>
        </div>
...
function ImportDialog({ open: isOpen, onClose, sessionId, connectionConfig, selectedDatabase }: Props) {
  ...
  const doImport = async () => {
    ...
    try {
      ...
      setResult({ ok: true, msg: t("dbClient.importComplete", "导入完成，耗时 {{ms}}ms", { ms: res.durationMs }) + (res.warnings.length ? t("dbClient.importWarnings", " ({{count}} 个警告)", { count: res.warnings.length }) : "") });
    } catch (e) { setResult({ ok: false, msg: String(e) }); }
    setRunning(false);
  };
    ...
    <Dialog open={isOpen} onOpenChange={v => { if (!v) onClose(); }}>
      <DialogContent className="!w-[400px] !max-w-[96vw] !gap-0 !p-0 !rounded-xl sm:!max-w-[400px]" showCloseButton>
        ...
        <div className="p-4 space-y-3">
          {!ok ? (
            <div className="rounded-md bg-warning/10 px-3 py-2 text-xs text-warning">{t("dbClient.importNotSupported", "当前数据库类型不支持导入")}</div>
          ) : (
            <>
              <Fld label={t("dbClient.importFile", "导入文件")}>
                <div className="flex gap-2">
              ...
              <div>
                ...
                {showAdv && (
                  <div className="mt-2 ml-4 border-l-2 border-border-default/30 pl-3">
                    <label className="block text-[11px] text-text-tertiary mb-1">{t("dbClient.toolPath", "工具路径")}</label>
                    <input value={toolPath} onChange={e => setToolPath(e.target.value)} placeholder={t("dbClient.autoDetect", "自动检测")}
              ...
              <button onClick={doImport} disabled={running || !importPath}
                className="flex w-full items-center justify-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50">
                {running ? <Loader2 size={14} className="animate-spin" /> : <Upload size={14} />} {t("dbClient.startImport", "开始导入")}
              </button>
            </>
          )}
        </div>
