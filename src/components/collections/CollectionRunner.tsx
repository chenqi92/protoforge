  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-bg-primary">
      ...
      <div className="shrink-0 flex items-center h-10 px-3 bg-bg-secondary/40 border-b border-border-default gap-2">
        ...
        {running ? (
          <div className="flex items-center gap-2 pf-text-xs text-accent">
            <div className="w-3 h-3 border-2 border-accent border-t-transparent rounded-full animate-spin" />
            {progress.index}/{progress.total}
          </div>
        ) : (
          <button onClick={handleRun} disabled={selectedIds.length === 0} className="h-7 px-4 rounded-md pf-text-sm font-semibold text-white bg-accent hover:bg-accent-hover disabled:opacity-50 flex items-center gap-1">
        ...
        <div className="w-64 shrink-0 border-r border-border-default flex flex-col overflow-hidden">
          ...
          <div className="p-3 border-b border-border-default">
            <div className="flex items-center justify-between mb-2">
              <h3 className="pf-text-xs font-bold text-text-disabled uppercase tracking-wider">{t('runner.requests')}</h3>
              <button onClick={toggleAll} className="pf-text-xxs text-accent hover:underline">
          ...
          <div className="p-3 space-y-3">
            <h3 className="pf-text-xs font-bold text-text-disabled uppercase tracking-wider flex items-center gap-1"><Settings2 className="w-3 h-3" /> {t('runner.config')}</h3>
            <div>
              <label className="pf-text-xs text-text-tertiary">{t('runner.iterations')}</label>
              <input type="number" min={1} max={100} value={iterations} onChange={(e) => setIterations(Math.max(1, parseInt(e.target.value) || 1))}
                className="w-full h-7 px-2 mt-1 pf-text-sm bg-bg-input border border-border-default rounded text-text-primary outline-none focus:border-accent" />
            </div>
            <div>
              <label className="pf-text-xs text-text-tertiary">{t('runner.delay')}</label>
          ...
          {summary && (
            <div className="mt-auto p-3 border-t border-border-default bg-bg-secondary/30">
              ...
              <div className="flex items-center justify-center mt-2 gap-1 pf-text-xs text-text-tertiary">
          ...
          {results.length === 0 ? (
            ...
            results.map((r, i) => (
              <div key={i} className={cn("flex items-center gap-3 px-3 py-2 rounded-md border transition-colors",
                ...
                {r.error && <span className="pf-text-xxs text-red-500 max-w-[120px] truncate" title={r.error}>{r.error}</span>}
              </div>
            ))
          )}
        </div>
