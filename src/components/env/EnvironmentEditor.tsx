    ...
    <div className="h-full flex flex-col overflow-hidden bg-bg-primary">
      ...
      <div className="shrink-0 px-5 pt-4 pb-0">
        <div className="flex items-center gap-3 mb-3">
          ...
          <h2 className="text-[15px] font-semibold text-text-primary">环境管理</h2>
        </div>
      ...
      {tab === 'environments' ? (
          ...
          <div className="w-56 shrink-0 border-r border-border-default flex flex-col">
            ...
            <div className="flex-1 overflow-auto py-1">
              {environments.length === 0 ? (
                <div className="p-4 text-center text-[12px] text-text-disabled">暂无环境</div>
              ) : (
            ...
            {activeEnvId && (
              <div className="shrink-0 px-3 py-2 border-t border-border-default text-[11px] text-emerald-600 font-medium flex items-center gap-1.5">
                ...
                激活: {environments.find((e) => e.id === activeEnvId)?.name || ''}
              </div>
            )}
          </div>
            ...
            {selectedEnvId ? (
              <>
                <div className="flex items-center justify-between px-4 py-2.5 border-b border-border-default bg-bg-secondary/30 shrink-0">
                  <span className="text-[13px] font-medium text-text-secondary">
                    {environments.find((e) => e.id === selectedEnvId)?.name} 的变量
                  </span>
                  <div className="flex items-center gap-2">
                    {dirty && <span className="text-[11px] text-amber-500 font-medium">● 未保存</span>}
                    <button
                ...
                <div className="flex-1 overflow-auto p-4">
                  ...
                  {editing.length > 0 && (
                    <div className="flex items-center gap-2 mb-2 px-8 text-[11px] font-semibold text-text-disabled uppercase tracking-wider">
                      ...
                      <div className="w-16 text-center">秘密</div>
                      <div className="w-8" />
                    </div>
                  )}
              ...
              <div className="flex-1 flex items-center justify-center text-text-disabled">
                <div className="text-center">
                  ...
                  <p className="text-[13px] text-text-secondary">选择一个环境</p>
                  <p className="text-[12px] mt-1">或在左侧创建新环境</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-border-default bg-bg-secondary/30 shrink-0">
            <span className="text-[13px] text-text-secondary">全局变量在所有环境中可用，优先级低于环境变量。</span>
            <button
