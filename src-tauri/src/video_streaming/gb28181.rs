...
use super::state::{Gb28181Session, ProtocolMessage};
...
fn generate_branch() -> String {
    format!("z9hG4bK{}", uuid::Uuid::new_v4().to_string().replace('-', "")[..16].to_string())
}
...
fn parse_sip_status(response: &str) -> (u16, String) {
    if let Some(first_line) = response.lines().next() {
        ...
        let code = parts.get(1).and_then(|c| c.parse::<u16>().ok()).unwrap_or(0);
...
async fn sip_send_recv(
    ...
    let recv_msg = ProtocolMessage {
        ...
        protocol: "gb28181".to_string(),
        summary: response.lines().next().unwrap_or("SIP response").to_string(),
...
pub async fn register(
    ...
    if status_code == 401 {
        ...
        let info_msg = ProtocolMessage {
            ...
            detail: "Server requires authentication. Re-sending REGISTER with credentials.".to_string(),
            timestamp: chrono::Utc::now().to_rfc3339(),
            size: None,
        };
    ...
    let info_msg = ProtocolMessage {
        ...
        detail: format!("Device ID: {}\nSIP Domain: {}\nServer: {}:{}", device_id, sip_domain, sip_server, sip_port),
        timestamp: chrono::Utc::now().to_rfc3339(),
        size: None,
    };
    let _ = app.emit("videostream-protocol-msg", &info_msg);
    ...
    Ok(Gb28181Session {
        ...
        call_id,
        cseq,
        transport: _transport.to_string(),
    })
}
        ...
        _ => {
            ...
            if status == 200 {
                ...
                let info_msg = ProtocolMessage {
                    ...
                    detail: "The server accepted the catalog query. Results may arrive asynchronously.".to_string(),
                    timestamp: chrono::Utc::now().to_rfc3339(),
                    size: None,
                };
                let _ = app.emit("videostream-protocol-msg", &info_msg);
...
fn build_ptz_command(direction: &str, speed: f64) -> String {
    ...
    cmd_bytes.iter().map(|b| format!("{:02X}", b)).collect::<String>()
}
