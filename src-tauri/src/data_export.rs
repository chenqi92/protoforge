...
/// 从数组前 N 行采样收集列名（API 返回的 JSON 数组结构一致，无需遍历全量）
fn collect_columns(arr: &[Value]) -> Vec<String> {
    let mut seen = indexmap::IndexSet::new();
    let sample = arr.len().min(20);
    for item in &arr[..sample] {
...
fn export_sql(
    ...
    // 字段选择：selectedColumns 逗号分隔，为空则全选
    let selected: Vec<&String> = match options.get("selectedColumns") {
        Some(s) if !s.trim().is_empty() => {
            let chosen: std::collections::HashSet<&str> = s.split(',').map(str::trim).collect();
            columns.iter().filter(|c| chosen.contains(c.as_str())).collect()
        }
        _ => columns.iter().collect(),
    };

    // 字段别名：columnAliases 是 JSON 对象 {"原名": "别名"}
    let aliases: std::collections::HashMap<String, String> = options
        .get("columnAliases")
        .and_then(|s| serde_json::from_str(s).ok())
        .unwrap_or_default();

    let mut buf = String::with_capacity(arr.len() * selected.len() * 30);
    ...
    let quote_col = |col: &str| -> String {
        let name = aliases.get(col).map(|a| a.as_str()).unwrap_or(col);
        if dialect == "mysql" { format!("`{}`", name) } else { format!("\"{}\"", name) }
    };
    let quoted_table = if dialect == "mysql" { format!("`{}`", table) } else { format!("\"{}\"", table) };
    let col_list = selected.iter().map(|c| quote_col(c)).collect::<Vec<_>>().join(", ");

    for row in arr {
        let values: Vec<String> = selected.iter().map(|col| {
            match row {
                Value::Object(map) => match map.get(col.as_str()) {
